...
const {mockData, mockOffer} = require(`./offer.test-data`);
...
const createAPI = () => {
  ...
  offer(app, new DataService(cloneData), mockRouter);
...
describe(`API returns an offer with given id`, () => {
  ...
  test(`Status code 200`, () => expect(response.statusCode).toBe(HttpCode.OK));
  test(`Offer title is "Продам отличную подборку фильмов на VHS."`, () => expect(response.body.title).toBe(`Продам отличную подборку фильмов на VHS.`));
});
